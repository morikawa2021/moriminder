# Mamorunder 詳細仕様書（設計書）

## 1. ドキュメント情報

### 1.1 ドキュメント概要
本ドキュメントは、Mamorunder（マモルンダー）の要求仕様書に基づいて作成された詳細仕様書（設計書）です。システムの技術的な設計、アーキテクチャ、実装詳細を定義します。
...
│  ┌─────────────────────────────┐   │
│  │ 📋 タスク名                  │   │
│  │ 🏷️ カテゴリ  ⭐ 重要度 🔄   │   │ ← 繰り返しタスクはアイコン表示
│  │ 📅 期限: 2025/01/15 15:00   │   │ ← 日時は強調表示
│  │ 🔔 リマインド: 1時間間隔     │   │
│  │ [スワイプ: 完了/削除]        │   │
│  └─────────────────────────────┘   │
│                                     │
...
※ 繰り返しタスクの親タスク（テンプレート）は表示されない
※ 子インスタンス（実際の予定日時を持つもの）のみ表示される
```
...
class TaskListViewModel: ObservableObject {
    ...
    func loadTasks() {
        // TaskManager.fetchTasks() は以下のフィルタリングを行う:
        // - 繰り返しタスクの親タスク（テンプレート）は除外
        // - 子インスタンス（実際の予定日時を持つもの）のみ取得
...
2. **フォアグラウンド復帰時** (MamorunderApp.swift):
   ```swift
...
1. **軽量マイグレーション（Lightweight Migration）を推奨:**
   ...
   - 実装:
     ```swift
     let container = NSPersistentContainer(name: "Mamorunder")
...
#### 15.1.1 概要
Notionで管理されているタスクとMamorunderの連動を検討する。カテゴリごとに個別のNotionデータベース（ページ）を指定して連携できる柔軟な設計とする。

#### 15.1.2 検討事項
- **同期方向**: 双方向同期（Mamorunder ↔ Notion）または一方向同期（Notion → Mamorunder）
- **同期対象**: 指定したカテゴリごとに、個別のNotionデータベース（ページ）を指定可能
- **認証**: Notion APIの認証方法（OAuth、内部統合トークンなど）
- **データマッピング**: MamorunderのタスクデータとNotionのデータベース構造の対応関係
