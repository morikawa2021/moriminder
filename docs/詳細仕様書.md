# Moriminder 詳細仕様書（設計書）

## 1. ドキュメント情報

### 1.1 ドキュメント概要
本ドキュメントは、Moriminder（モリマインダー）の要求仕様書に基づいて作成された詳細仕様書（設計書）です。システムの技術的な設計、アーキテクチャ、実装詳細を定義します。

### 1.2 バージョン情報
| バージョン | 日付 | 変更内容 | 変更者 |
|-----------|------|---------|--------|
| 1.0 | 2025-11-09 02:23 | 初版作成 | - |

### 1.3 関連ドキュメント
- 要求仕様書.md

---

## 2. システムアーキテクチャ

### 2.1 全体アーキテクチャ

#### 2.1.1 アーキテクチャパターン
- **パターン**: MVVM（Model-View-ViewModel）パターン
- **理由**: SwiftUIとの親和性が高く、テスト容易性と保守性を確保

#### 2.1.2 レイヤー構成
```
┌─────────────────────────────────────┐
│         Presentation Layer          │
│  (Views, ViewModels, ViewModifiers) │
└─────────────────────────────────────┘
              ↓
┌─────────────────────────────────────┐
│          Business Logic Layer       │
│  (UseCases, Services, Managers)     │
└─────────────────────────────────────┘
              ↓
┌─────────────────────────────────────┐
│          Data Layer                 │
│  (Repositories, DataSources)       │
└─────────────────────────────────────┘
              ↓
┌─────────────────────────────────────┐
│          Persistence Layer          │
│  (Core Data / SwiftData)            │
└─────────────────────────────────────┘
```

### 2.2 主要コンポーネント

#### 2.2.1 コンポーネント一覧
1. **TaskManager**: タスクのCRUD操作を管理
2. **NotificationManager**: 通知のスケジューリングと管理
3. **ReminderService**: リマインドロジックの実行
4. **AlarmService**: アラームの管理
5. **CategoryManager**: カテゴリの管理
6. **TaskSubdivisionService**: タスク細分化機能（LLM API連携）
7. **NaturalLanguageParser**: 自然言語からの日時抽出
8. **RepeatingTaskGenerator**: 繰り返しタスクの生成

---

## 3. データモデル設計

### 3.1 エンティティ設計

#### 3.1.1 Task（タスク）
```swift
class Task: NSManagedObject {
    // 基本情報
    @NSManaged var id: UUID
    @NSManaged var title: String              // タスク名（必須）
    @NSManaged var createdAt: Date            // 登録日時
    @NSManaged var completedAt: Date?         // 完了日時
    
    // 分類
    @NSManaged var category: Category?        // カテゴリ（任意）
    @NSManaged var priority: Priority         // 重要度（低/中/高）
    
    // 日時設定
    @NSManaged var taskType: TaskType         // タスク/スケジュール
    @NSManaged var deadline: Date?            // 期限（タスクの場合）
    @NSManaged var startDateTime: Date?       // 開始日時（スケジュールの場合）
    
    // 通知設定
    @NSManaged var alarmDateTime: Date?       // アラーム時刻
    @NSManaged var alarmSound: String?        // アラーム音
    @NSManaged var alarmEnabled: Bool         // アラーム有効/無効
    
    // リマインド設定
    @NSManaged var reminderEnabled: Bool      // リマインド有効/無効
    @NSManaged var reminderInterval: Int      // リマインド間隔（分）
    @NSManaged var reminderStartTime: Date?   // リマインド開始時刻
    @NSManaged var reminderEndTime: Date?     // リマインド終了時刻
    @NSManaged var snoozeMaxCount: Int        // スヌーズ最大回数（デフォルト5、最大10）
    @NSManaged var snoozeUnlimited: Bool      // 無期限スヌーズ（期限超過時）
    @NSManaged var snoozeCount: Int           // 現在のスヌーズ回数
    @NSManaged var lastSnoozeDateTime: Date?  // 最後のスヌーズ日時（リセット判定用）
    
    // 繰り返し設定
    @NSManaged var isRepeating: Bool          // 繰り返しタスクかどうか
    @NSManaged var repeatPattern: RepeatPattern? // 繰り返しパターン
    @NSManaged var repeatEndDate: Date?       // 繰り返し終了日時
    @NSManaged var parentTaskId: UUID?         // 親タスクID（繰り返しタスクの場合）
    
    // 状態
    @NSManaged var isCompleted: Bool           // 完了状態
    @NSManaged var isArchived: Bool           // アーカイブ状態
}
```

#### 3.1.2 Category（カテゴリ）
```swift
class Category: NSManagedObject {
    @NSManaged var id: UUID
    @NSManaged var name: String               // カテゴリ名
    @NSManaged var color: String              // カテゴリの色（HEXコード）
    @NSManaged var createdAt: Date
    @NSManaged var usageCount: Int            // 使用回数（よく使うカテゴリの判定用）
    @NSManaged var tasks: NSSet?              // 関連タスク
}
```

#### 3.1.3 NotificationRecord（通知記録）
```swift
class NotificationRecord: NSManagedObject {
    @NSManaged var id: UUID
    @NSManaged var taskId: UUID               // 関連タスクID
    @NSManaged var notificationId: String     // ローカル通知ID
    @NSManaged var scheduledTime: Date        // スケジュール時刻
    @NSManaged var notificationType: NotificationType // アラーム/リマインド
    @NSManaged var isDelivered: Bool          // 配信済みかどうか
    @NSManaged var deliveredAt: Date?         // 配信日時
}
```

#### 3.1.4 PresetTime（プリセット時間）
```swift
class PresetTime: NSManagedObject {
    @NSManaged var id: UUID
    @NSManaged var name: String               // プリセット名（例: "今日の9時"）
    @NSManaged var hour: Int                  // 時
    @NSManaged var minute: Int                // 分
    @NSManaged var offsetDays: Int            // 日付オフセット（0=今日、1=明日など）
    @NSManaged var isDefault: Bool            // デフォルトプリセットかどうか
    @NSManaged var order: Int                 // 表示順序
}
```

### 3.2 列挙型定義

#### 3.2.1 TaskType
```swift
enum TaskType: String, Codable {
    case task = "task"           // タスク（期限設定）
    case schedule = "schedule"   // スケジュール（開始日時設定）
}
```

#### 3.2.2 Priority
```swift
enum Priority: String, Codable {
    case low = "low"     // 低
    case medium = "medium" // 中
    case high = "high"   // 高
}
```

#### 3.2.3 RepeatPattern
```swift
// 繰り返しパターンのタイプ
enum RepeatType: String, Codable {
    case daily = "daily"                     // 毎日
    case weekly = "weekly"                   // 毎週
    case monthly = "monthly"                 // 毎月
    case yearly = "yearly"                   // 毎年
    case everyNDays = "everyNDays"           // N日ごと
    case nthWeekdayOfMonth = "nthWeekdayOfMonth" // 毎月第N曜日
    case custom = "custom"                   // カスタム
}

// 繰り返しパターン（パラメータを含む）
struct RepeatPattern: Codable {
    let type: RepeatType                     // パターンタイプ
    let interval: Int?                       // N日ごと の N（everyNDaysの場合）
    let weekday: Int?                        // 曜日（1=日曜日〜7=土曜日、nthWeekdayOfMonthの場合）
    let week: Int?                           // 第N週（1=第1週、nthWeekdayOfMonthの場合）
    let customDays: [Int]?                   // カスタムパターンの曜日（customの場合）

    // 便利なイニシャライザ
    static func daily() -> RepeatPattern {
        return RepeatPattern(type: .daily, interval: nil, weekday: nil, week: nil, customDays: nil)
    }

    static func weekly() -> RepeatPattern {
        return RepeatPattern(type: .weekly, interval: nil, weekday: nil, week: nil, customDays: nil)
    }

    static func monthly() -> RepeatPattern {
        return RepeatPattern(type: .monthly, interval: nil, weekday: nil, week: nil, customDays: nil)
    }

    static func yearly() -> RepeatPattern {
        return RepeatPattern(type: .yearly, interval: nil, weekday: nil, week: nil, customDays: nil)
    }

    static func everyNDays(_ n: Int) -> RepeatPattern {
        return RepeatPattern(type: .everyNDays, interval: n, weekday: nil, week: nil, customDays: nil)
    }

    static func nthWeekdayOfMonth(weekday: Int, week: Int) -> RepeatPattern {
        return RepeatPattern(type: .nthWeekdayOfMonth, interval: nil, weekday: weekday, week: week, customDays: nil)
    }

    static func custom(days: [Int]) -> RepeatPattern {
        return RepeatPattern(type: .custom, interval: nil, weekday: nil, week: nil, customDays: days)
    }
}
```

#### 3.2.4 NotificationType
```swift
enum NotificationType: String, Codable {
    case alarm = "alarm"         // アラーム
    case reminder = "reminder"    // リマインド
}
```

### 3.3 データベーススキーマ

#### 3.3.1 Core Data エンティティリレーション
```
Task
├── category (Category) [Optional, To-One]
├── repeatPattern (RepeatPattern) [Optional, Transformable]
└── notifications (NotificationRecord) [To-Many]

Category
└── tasks (Task) [To-Many]

NotificationRecord
└── task (Task) [To-One]
```

**RepeatPatternの保存方法:**
- Core Dataでは`Transformable`属性として保存
- `RepeatPattern`は`Codable`に準拠したstructのため、`NSSecureUnarchiveFromDataTransformer`を使用してシリアライズ
- Core Dataモデルエディタでの設定:
  - 属性名: `repeatPattern`
  - 型: `Transformable`
  - Custom Class: `RepeatPattern`
  - Transformer: `NSSecureUnarchiveFromDataTransformer`
  - Optional: `true`
- 実装例:
  ```swift
  // Task+CoreDataProperties.swift
  @NSManaged var repeatPattern: RepeatPattern?

  // RepeatPattern は Codable に準拠しているため、
  // NSSecureUnarchiveFromDataTransformer が自動的に
  // エンコード・デコード処理を行う
  ```
- メリット:
  - シンプルで実装が容易
  - RepeatPatternの構造を変更してもCore Dataスキーマの変更が不要
  - 個別の属性に分解する方法よりもコード量が少ない

---

## 4. 画面設計

### 4.1 画面一覧

| 画面ID | 画面名 | 説明 |
|--------|--------|------|
| S001 | タスク一覧画面 | 登録されたタスクを一覧表示 |
| S002 | タスク登録画面 | 新規タスクを登録 |
| S003 | タスク編集画面 | 既存タスクを編集 |
| S004 | タスク詳細画面 | タスクの詳細情報を表示 |
| S005 | 細分化提案画面 | LLMによるタスク細分化提案を表示 |
| S006 | カテゴリ管理画面 | カテゴリの一覧・編集・削除 |
| S007 | 設定画面 | アプリ設定 |

### 4.2 画面詳細設計

#### 4.2.1 S001: タスク一覧画面

**画面構成**
```
┌─────────────────────────────────────┐
│  [フィルタ] [ソート] [検索]         │ ← ヘッダー
├─────────────────────────────────────┤
│                                     │
│  ┌─────────────────────────────┐   │
│  │ 📋 タスク名                  │   │
│  │ 🏷️ カテゴリ  ⭐ 重要度      │   │
│  │ 📅 期限: 2025/01/15 15:00   │   │
│  │ 🔔 リマインド: 1時間間隔     │   │
│  │ [スワイプ: 完了/削除]        │   │
│  └─────────────────────────────┘   │
│                                     │
│  ┌─────────────────────────────┐   │
│  │ 📋 別のタスク                │   │
│  │ ...                          │   │
│  └─────────────────────────────┘   │
│                                     │
├─────────────────────────────────────┤
│        [+ 新規タスク追加]           │ ← 固定ボタン
└─────────────────────────────────────┘
```

**View構造**
```swift
struct TaskListView: View {
    @StateObject private var viewModel: TaskListViewModel
    @State private var filterMode: FilterMode = .all
    @State private var sortMode: SortMode = .createdAtDesc
    
    var body: some View {
        NavigationView {
            VStack {
                // フィルタ・ソートバー
                FilterSortBar(...)
                
                // タスクリスト
                List {
                    ForEach(viewModel.tasks) { task in
                        TaskCardView(task: task)
                            .swipeActions(edge: .trailing) {
                                // 完了アクション
                                Button {
                                    viewModel.completeTask(task)
                                } label: {
                                    Label("完了", systemImage: "checkmark")
                                }
                                .tint(.green)
                                
                                // 削除アクション
                                Button(role: .destructive) {
                                    viewModel.deleteTask(task)
                                } label: {
                                    Label("削除", systemImage: "trash")
                                }
                            }
                    }
                }
                
                // 新規タスク追加ボタン
                Button {
                    viewModel.showAddTask = true
                } label: {
                    Label("新規タスク追加", systemImage: "plus.circle.fill")
                        .font(.title2)
                }
                .padding()
            }
            .navigationTitle("タスク一覧")
            .sheet(isPresented: $viewModel.showAddTask) {
                TaskEditView(mode: .create)
            }
        }
    }
}
```

**ViewModel**
```swift
class TaskListViewModel: ObservableObject {
    @Published var tasks: [Task] = []
    @Published var filterMode: FilterMode = .all
    @Published var sortMode: SortMode = .createdAtDesc
    @Published var showAddTask = false
    
    private let taskManager: TaskManager
    
    func loadTasks() {
        tasks = taskManager.fetchTasks(
            filter: filterMode,
            sort: sortMode
        )
    }
    
    func completeTask(_ task: Task) {
        // リマインド中のタスクの場合は確認ダイアログを表示
        if task.reminderEnabled {
            // 確認ダイアログ表示
        } else {
            taskManager.completeTask(task)
        }
    }
    
    func deleteTask(_ task: Task) {
        taskManager.deleteTask(task)
        loadTasks()
    }
}
```

#### 4.2.2 S002/S003: タスク登録・編集画面

**画面構成**
```
┌─────────────────────────────────────┐
│  [キャンセル]  タスク登録  [保存]   │ ← ナビゲーションバー
├─────────────────────────────────────┤
│                                     │
│  ┌─────────────────────────────┐   │
│  │ タスク名 *                  │   │
│  │ [________________________]  │   │
│  └─────────────────────────────┘   │
│                                     │
│  ┌─────────────────────────────┐   │
│  │ カテゴリ                    │   │
│  │ [オートコンプリート入力]    │   │
│  └─────────────────────────────┘   │
│                                     │
│  ┌─────────────────────────────┐   │
│  │ 重要度                      │   │
│  │ [低] [中] [高]              │   │
│  └─────────────────────────────┘   │
│                                     │
│  ▼ 日時設定                         │
│  ┌─────────────────────────────┐   │
│  │ [期限] [開始日時]            │   │
│  │ [期限を設定する] [ON/OFF]    │   │
│  │                             │   │
│  │ プリセット時間:              │   │
│  │ [今日9時] [今日12時] ...    │   │
│  │                             │   │
│  │ 期限: [DatePicker]           │   │
│  │ ※ONの場合のみ表示            │   │
│  └─────────────────────────────┘   │
│                                     │
│  ▼ アラーム設定                     │
│  ┌─────────────────────────────┐   │
│  │ [ON/OFF]                    │   │
│  │ アラーム時刻: [設定]        │   │
│  │ ※デフォルトは期限設定の時刻 │   │
│  └─────────────────────────────┘   │
│                                     │
│  ▼ リマインド設定                   │
│  ┌─────────────────────────────┐   │
│  │ [ON/OFF]                    │   │
│  │ 間隔: [1時間]               │   │
│  │ スヌーズ最大回数: [5回]     │   │
│  └─────────────────────────────┘   │
│                                     │
│  ▼ 繰り返し設定                     │
│  ┌─────────────────────────────┐   │
│  │ [ON/OFF]                    │   │
│  │ パターン: [毎日] [毎週] ... │   │
│  └─────────────────────────────┘   │
│                                     │
└─────────────────────────────────────┘
```

**View構造**
```swift
struct TaskEditView: View {
    enum Mode {
        case create
        case edit(Task)
    }
    
    let mode: Mode
    @StateObject private var viewModel: TaskEditViewModel
    @Environment(\.dismiss) private var dismiss
    
    var body: some View {
        NavigationView {
            Form {
                // タスク名
                Section {
                    TextField("タスク名", text: $viewModel.title)
                        .font(.title3)
                }
                
                // カテゴリ
                Section("カテゴリ") {
                    CategoryPickerView(
                        selectedCategory: $viewModel.category,
                        categories: viewModel.categories
                    )
                }
                
                // 重要度
                Section("重要度") {
                    Picker("重要度", selection: $viewModel.priority) {
                        Text("低").tag(Priority.low)
                        Text("中").tag(Priority.medium)
                        Text("高").tag(Priority.high)
                    }
                    .pickerStyle(.segmented)
                }
                
                // 日時設定
                Section {
                    DateSettingView(
                        taskType: $viewModel.taskType,
                        deadline: $viewModel.deadline,
                        startDateTime: $viewModel.startDateTime,
                        presetTimes: viewModel.presetTimes
                    )
                }
                
                // アラーム設定
                Section {
                    AlarmSettingView(
                        enabled: $viewModel.alarmEnabled,
                        dateTime: $viewModel.alarmDateTime,
                        sound: $viewModel.alarmSound,
                        defaultDateTime: viewModel.taskType == .task ? viewModel.deadline : viewModel.startDateTime
                    )
                }
                
                // リマインド設定
                Section {
                    ReminderSettingView(
                        enabled: $viewModel.reminderEnabled,
                        interval: $viewModel.reminderInterval,
                        priority: viewModel.priority,
                        taskType: viewModel.taskType,
                        snoozeMaxCount: $viewModel.snoozeMaxCount,
                        snoozeUnlimited: $viewModel.snoozeUnlimited,
                        reminderEndTime: $viewModel.reminderEndTime,
                        deadline: viewModel.deadline,
                        startDateTime: viewModel.startDateTime
                    )
                }
                
                // 繰り返し設定
                Section {
                    RepeatSettingView(
                        enabled: $viewModel.isRepeating,
                        pattern: $viewModel.repeatPattern,
                        endDate: $viewModel.repeatEndDate
                    )
                }
            }
            .navigationTitle(mode == .create ? "タスク登録" : "タスク編集")
            .navigationBarTitleDisplayMode(.inline)
            .toolbar {
                ToolbarItem(placement: .cancellationAction) {
                    Button("キャンセル") {
                        dismiss()
                    }
                }
                ToolbarItem(placement: .confirmationAction) {
                    Button("保存") {
                        viewModel.save()
                        dismiss()
                    }
                    .disabled(!viewModel.isValid)
                }
            }
        }
    }
}
```

**自然言語解析の統合**
```swift
struct TaskNameInputView: View {
    @Binding var title: String
    @Binding var extractedDateTime: Date?
    @Binding var extractedTaskType: TaskType?
    
    @StateObject private var parser = NaturalLanguageParser()
    
    var body: some View {
        TextField("タスク名", text: $title)
            .onChange(of: title) { newValue in
                parseNaturalLanguage(newValue)
            }
    }
    
    private func parseNaturalLanguage(_ text: String) {
        let result = parser.extractDateTime(from: text)
        if let dateTime = result.dateTime {
            extractedDateTime = dateTime
            extractedTaskType = result.taskType
            // 抽出結果をユーザーに表示
        }
    }
}
```

**DateSettingViewの実装**
```swift
struct DateSettingView: View {
    @Binding var taskType: TaskType
    @Binding var deadline: Date?
    @Binding var startDateTime: Date?
    let presetTimes: [PresetTime]
    
    // 期限/開始日時が設定されているかどうか
    private var isDateTimeEnabled: Bool {
        if taskType == .task {
            return deadline != nil
        } else {
            return startDateTime != nil
        }
    }
    
    var body: some View {
        Group {
            // タスクタイプの選択
            Picker("種類", selection: $taskType) {
                Text("期限").tag(TaskType.task)
                Text("開始日時").tag(TaskType.schedule)
            }
            .pickerStyle(.segmented)
            
            // 期限/開始日時を設定するかどうかのトグル
            Toggle(
                taskType == .task ? "期限を設定する" : "開始日時を設定する",
                isOn: Binding(
                    get: { isDateTimeEnabled },
                    set: { enabled in
                        if enabled {
                            // 有効にする場合、デフォルト値を設定
                            if taskType == .task {
                                deadline = Date()
                            } else {
                                startDateTime = Date()
                            }
                        } else {
                            // 無効にする場合、nilに設定
                            if taskType == .task {
                                deadline = nil
                            } else {
                                startDateTime = nil
                            }
                        }
                    }
                )
            )
            
            // プリセット時間の表示（トグルがOFFでも表示）
            if !presetTimes.isEmpty {
                ScrollView(.horizontal, showsIndicators: false) {
                    HStack(spacing: 12) {
                        ForEach(presetTimes, id: \.objectID) { preset in
                            PresetTimeButton(
                                preset: preset,
                                taskType: taskType,
                                deadline: $deadline,
                                startDateTime: $startDateTime
                            )
                        }
                    }
                    .padding(.horizontal)
                }
            }
            
            // 設定が有効な場合のみDatePickerを表示
            if isDateTimeEnabled {
                if taskType == .task {
                    DatePicker("期限", selection: Binding(
                        get: { deadline ?? Date() },
                        set: { deadline = $0 }
                    ), displayedComponents: [.date, .hourAndMinute])
                } else {
                    DatePicker("開始日時", selection: Binding(
                        get: { startDateTime ?? Date() },
                        set: { startDateTime = $0 }
                    ), displayedComponents: [.date, .hourAndMinute])
                }
            }
        }
        .environment(\.locale, Locale(identifier: "ja_JP"))
    }
}
```

**機能詳細**
- **日時設定の任意項目としての実装**:
  - 日時設定は任意項目であり、デフォルトでは未設定（nil）の状態
  - 「期限を設定する」/「開始日時を設定する」トグルで有効/無効を切り替え可能
  - トグルがOFFの場合: 期限/開始日時は未設定（nil）のまま、DatePickerは非表示
  - トグルがONの場合: DatePickerが表示され、デフォルト値として現在時刻が設定される
  - プリセット時間ボタンはトグルがOFFでも表示され、タップすると自動的にトグルがONになり値が設定される
  - トグルをOFFにすると、設定されていた値がnilにリセットされる
  - これにより、ユーザーが明示的に設定しない限り、期限/開始日時は設定されない

- **アラーム設定のデフォルト値**:
  - アラーム設定を有効にした際、アラーム時刻のデフォルト値は期限設定で設定した時刻を使用する
  - タスクタイプが「期限」の場合: `deadline`をデフォルト値として使用
  - タスクタイプが「開始日時」の場合: `startDateTime`をデフォルト値として使用
  - 期限設定が未設定の場合のみ、現在時刻をデフォルト値として使用
  - これにより、ユーザーは期限設定の時刻を基準にアラーム時刻を設定でき、操作が直感的になる

- **リマインド設定の動作**:
  - **デフォルト設定を使用**がONの場合:
    - 重要度とタスクタイプに応じたデフォルト間隔が表示される
    - 間隔の選択UI（Picker）は非表示
    - 保存時にデフォルト間隔が`task.reminderInterval`に設定される
    - デフォルト間隔は以下の通り:
      - 低重要度・タスク: 24時間間隔（1440分）
      - 中重要度・タスク: 3時間間隔（180分）
      - 高重要度・タスク: 1時間間隔（60分）
      - スケジュールタイプ: 段階的リマインド（最初の間隔は60分）
  - **デフォルト設定を使用**がOFFの場合:
    - 間隔の選択UI（Picker）が表示される
    - ユーザーが選択した間隔（5分、15分、30分、1時間、3時間、6時間、12時間、24時間）が`task.reminderInterval`に設定される
  - **既存タスク編集時**:
    - `onAppear`で、既存の間隔がデフォルト値と一致するかどうかで「デフォルト設定を使用」の状態を自動判定
    - 一致する場合は`useDefaultSettings = true`、一致しない場合は`useDefaultSettings = false`
  - **リマインド開始時刻の決定**:
    - `reminderStartTime`が明示的に設定されている場合: その時刻から開始
    - 設定されていない場合: 期限時刻（または開始日時）から逆算してリマインドを開始
    - 期限時刻から逆算する場合、最初の間隔分だけ期限時刻より前に開始

#### 4.2.3 S006: カテゴリ管理画面

**画面構成**
```
┌─────────────────────────────────────┐
│  [閉じる]  カテゴリ管理             │ ← ナビゲーションバー
├─────────────────────────────────────┤
│                                     │
│  ┌─────────────────────────────┐   │
│  │ 🟦 カテゴリ名               │   │
│  │    5回使用  3件のタスク     │   │
│  │    [編集] [スワイプ: 削除]  │   │
│  └─────────────────────────────┘   │
│                                     │
│  ┌─────────────────────────────┐   │
│  │ 🟥 別のカテゴリ              │   │
│  │    ...                       │   │
│  └─────────────────────────────┘   │
│                                     │
└─────────────────────────────────────┘
```

**View構造**
```swift
struct CategoryManagementView: View {
    @StateObject private var viewModel: CategoryManagementViewModel
    @Environment(\.managedObjectContext) private var viewContext
    @Environment(\.dismiss) private var dismiss
    
    var body: some View {
        NavigationView {
            VStack {
                // ヘッダー
                AppHeaderView(screenTitle: "カテゴリ管理")
                
                // カテゴリリスト
                if viewModel.categories.isEmpty {
                    // 空状態の表示
                    VStack {
                        Image(systemName: "folder.badge.questionmark")
                        Text("カテゴリがありません")
                    }
                } else {
                    List {
                        ForEach(viewModel.categories) { category in
                            CategoryManagementRow(
                                category: category,
                                viewModel: viewModel
                            )
                            .swipeActions(edge: .trailing) {
                                Button(role: .destructive) {
                                    viewModel.categoryToDelete = category
                                } label: {
                                    Label("削除", systemImage: "trash")
                                }
                            }
                        }
                    }
                }
            }
            .navigationBarTitleDisplayMode(.inline)
            .toolbar {
                ToolbarItem(placement: .cancellationAction) {
                    Button("閉じる") { dismiss() }
                }
            }
            .sheet(item: $viewModel.categoryToEdit) { category in
                CategoryEditView(...)
            }
            .alert("カテゴリを削除しますか？", ...) {
                // 削除確認ダイアログ
            }
        }
    }
}
```

**ViewModel**
```swift
class CategoryManagementViewModel: ObservableObject {
    @Published var categories: [Category] = []
    @Published var categoryToDelete: Category?
    @Published var categoryToEdit: Category?
    @Published var editingName: String = ""
    @Published var editingColor: String = "#007AFF"
    
    private let viewContext: NSManagedObjectContext
    private var categoryManager: CategoryManager {
        CategoryManager(viewContext: viewContext)
    }
    
    func loadCategories() {
        categories = categoryManager.fetchCategories()
    }
    
    func deleteCategory(_ category: Category) {
        try? categoryManager.deleteCategory(category)
        loadCategories()
    }
    
    func startEditing(_ category: Category) {
        categoryToEdit = category
        editingName = category.name ?? ""
        editingColor = category.color ?? "#007AFF"
    }
    
    func saveEditing() {
        guard let category = categoryToEdit else { return }
        if editingName != category.name {
            try? categoryManager.updateCategoryName(category, name: editingName)
        }
        if editingColor != category.color {
            try? categoryManager.updateCategoryColor(category, color: editingColor)
        }
        loadCategories()
        categoryToEdit = nil
    }
}
```

**機能詳細**
- **カテゴリ一覧表示**:
  - 使用回数順にソートして表示
  - 各カテゴリに色、名前、使用回数、関連タスク数を表示
  - カテゴリが存在しない場合は空状態を表示
- **カテゴリ編集**:
  - カテゴリ行の編集ボタンから編集画面を開く
  - カテゴリ名の変更
  - カテゴリの色の変更（プリセット色から選択）
- **カテゴリ削除**:
  - スワイプアクションまたは編集画面から削除可能
  - 削除前に確認ダイアログを表示
  - 関連タスクがある場合は警告を表示
- **画面遷移**:
  - タスク一覧画面のナビゲーションバーからアクセス可能
  - モーダルシートで表示

#### 4.2.4 S005: 細分化提案画面

**画面構成**
```
┌─────────────────────────────────────┐
│  [閉じる]  タスク細分化             │
├─────────────────────────────────────┤
│                                     │
│  元のタスク:                        │
│  「部屋の掃除」                     │
│                                     │
│  ┌─────────────────────────────┐   │
│  │ [処理中...]                 │   │ ← LLM処理中
│  └─────────────────────────────┘   │
│                                     │
│  提案された細分化タスク:             │
│  ┌─────────────────────────────┐   │
│  │ ☑ 床を掃除機でかける        │   │
│  │ ☑ 窓を拭く                  │   │
│  │ ☑ ゴミを捨てる              │   │
│  │ ☐ 机を整理する              │   │
│  └─────────────────────────────┘   │
│                                     │
│  [承認] [修正] [却下]               │
│                                     │
└─────────────────────────────────────┘
```

**View構造**
```swift
struct TaskSubdivisionView: View {
    let originalTask: Task
    @StateObject private var viewModel: TaskSubdivisionViewModel
    @Environment(\.dismiss) private var dismiss
    
    var body: some View {
        NavigationView {
            VStack {
                // 元のタスク表示
                Text("元のタスク:")
                    .font(.headline)
                Text(originalTask.title)
                    .font(.title2)
                    .padding()
                
                // ローディングまたは提案リスト
                if viewModel.isLoading {
                    ProgressView("処理中...")
                        .padding()
                } else if let proposals = viewModel.proposals {
                    List {
                        ForEach(proposals) { proposal in
                            HStack {
                                Toggle("", isOn: Binding(
                                    get: { proposal.isSelected },
                                    set: { viewModel.toggleProposal(proposal, isSelected: $0) }
                                ))
                                Text(proposal.title)
                            }
                        }
                    }
                } else if let error = viewModel.error {
                    VStack {
                        Text("エラーが発生しました")
                            .foregroundColor(.red)
                        Text(error.localizedDescription)
                        Button("再試行") {
                            viewModel.generateProposals()
                        }
                    }
                }
                
                // アクションボタン
                HStack {
                    Button("却下") {
                        dismiss()
                    }
                    .buttonStyle(.bordered)
                    
                    Button("修正") {
                        viewModel.showEditMode = true
                    }
                    .buttonStyle(.bordered)
                    
                    Button("承認") {
                        viewModel.approveProposals()
                        dismiss()
                    }
                    .buttonStyle(.borderedProminent)
                    .disabled(viewModel.proposals?.isEmpty ?? true)
                }
                .padding()
            }
            .navigationTitle("タスク細分化")
            .navigationBarTitleDisplayMode(.inline)
            .toolbar {
                ToolbarItem(placement: .cancellationAction) {
                    Button("閉じる") {
                        dismiss()
                    }
                }
            }
            .onAppear {
                viewModel.generateProposals(for: originalTask)
            }
        }
    }
}
```

---

## 5. ビジネスロジック設計

### 5.1 タスク管理ロジック

#### 5.1.1 TaskManager

```swift
class TaskManager {
    private let repository: TaskRepository
    private let notificationManager: NotificationManager
    
    // タスク作成
    func createTask(_ task: Task) async throws {
        // 1. バリデーション
        try validateTask(task)
        
        // 2. 保存
        try await repository.save(task)
        
        // 3. 通知スケジュール
        if task.alarmEnabled {
            try await notificationManager.scheduleAlarm(for: task)
        }
        if task.reminderEnabled {
            try await notificationManager.scheduleReminder(for: task)
        }
        
        // 4. 繰り返しタスクの場合は次回インスタンスを生成
        if task.isRepeating {
            try await generateNextRepeatInstance(for: task)
        }
    }
    
    // タスク完了
    func completeTask(_ task: Task) async throws {
        // 1. 完了確認（リマインド中の場合は確認ダイアログが必要）
        if task.reminderEnabled && !task.isCompleted {
            // 確認ダイアログはUI層で処理
        }
        
        // 2. タスクを完了状態に更新
        task.isCompleted = true
        task.completedAt = Date()
        
        // 3. 通知をキャンセル
        try await notificationManager.cancelNotifications(for: task)
        
        // 4. 保存
        try await repository.save(task)
        
        // 5. 繰り返しタスクの場合は次回インスタンスを生成
        if task.isRepeating {
            try await generateNextRepeatInstance(for: task)
        }
    }
    
    // タスク削除
    func deleteTask(_ task: Task) async throws {
        // 1. 通知をキャンセル
        try await notificationManager.cancelNotifications(for: task)
        
        // 2. 削除
        try await repository.delete(task)
    }
    
    // バリデーション
    private func validateTask(_ task: Task) throws {
        guard !task.title.isEmpty else {
            throw TaskError.invalidTitle
        }
        
        // 日時設定のバリデーション
        if let deadline = task.deadline, let startDateTime = task.startDateTime {
            guard deadline >= startDateTime else {
                throw TaskError.invalidDateTime
            }
        }
    }
}
```

### 5.2 リマインドロジック

#### 5.2.1 ReminderService

```swift
class ReminderService {
    private let notificationManager: NotificationManager
    
    // リマインドスケジュール（iOS通知64個制限に対応）
    func scheduleReminder(for task: Task) async throws {
        guard task.reminderEnabled else { return }

        let intervals = calculateReminderIntervals(for: task)
        
        // 開始時刻の決定ロジック
        // 1. reminderStartTimeが明示的に設定されている場合はそれを使用
        // 2. 設定されていない場合、期限時刻（または開始日時）を基準にリマインドを開始
        var startTime: Date
        if let explicitStartTime = task.reminderStartTime {
            startTime = explicitStartTime
            if startTime < Date() {
                print("警告: リマインド開始時刻が過去です。現在時刻から開始します: \(startTime)")
                startTime = Date()
            }
        } else {
            // reminderStartTimeが設定されていない場合、期限時刻または開始日時を基準に開始
            let targetTime = task.deadline ?? task.startDateTime
            if let targetTime = targetTime {
                // 期限時刻（または開始日時）から逆算してリマインドを開始
                // 最初の間隔を取得して、期限時刻の前に開始
                let firstInterval = intervals.first ?? 180 // デフォルト3時間
                startTime = targetTime.addingTimeInterval(-TimeInterval(firstInterval * 60))
                
                // 開始時刻が現在時刻より過去の場合は、現在時刻から開始
                if startTime < Date() {
                    print("警告: 計算されたリマインド開始時刻が過去です。現在時刻から開始します: \(startTime)")
                    startTime = Date()
                }
            } else {
                // 期限時刻も開始日時もない場合、現在時刻から開始
                startTime = Date()
            }
        }
        
        let endTime = task.reminderEndTime ?? task.deadline ?? task.startDateTime

        // 終了日時がない場合、無限に通知をスケジュールするため、より多くの通知をスケジュール
        // iOS の通知64個制限に対応するため、直近の通知のみをスケジュール
        // 重要度に応じて最大通知数を調整
        let maxNotificationsPerTask: Int
        if endTime == nil {
            // 終了日時がない場合: より多くの通知をスケジュール（64個制限内で可能な限り多く）
            // ただし、他のタスクとのバランスを考慮して、重要度に応じた上限を設定
            switch task.priority {
            case .high:
                maxNotificationsPerTask = 30  // 高重要度: 最大30個（終了日時なし）
            case .medium:
                maxNotificationsPerTask = 20  // 中重要度: 最大20個（終了日時なし）
            case .low:
                maxNotificationsPerTask = 10   // 低重要度: 最大10個（終了日時なし）
            }
        } else {
            // 終了日時がある場合: 従来通り
            switch task.priority {
            case .high:
                maxNotificationsPerTask = 15  // 高重要度: 最大15個
            case .medium:
                maxNotificationsPerTask = 10  // 中重要度: 最大10個
            case .low:
                maxNotificationsPerTask = 5   // 低重要度: 最大5個
            }
        }

        // 期限時刻（または開始日時）を基準にリマインドを設定する場合の処理
        let targetTime = task.deadline ?? task.startDateTime
        let shouldCalculateFromTarget = targetTime != nil && task.reminderStartTime == nil
        
        if shouldCalculateFromTarget, let targetTime = targetTime {
            // 期限時刻から逆算してリマインドを設定
            var reminderTimes: [Date] = []
            var accumulatedInterval: TimeInterval = 0
            
            // リマインド終了時刻を取得（設定されている場合）
            let reminderEndTime = task.reminderEndTime
            
            // 期限時刻から逆算してリマインド時刻を計算
            for i in 0..<maxNotificationsPerTask {
                let intervalIndex = i % intervals.count
                let intervalMinutes = intervals[intervalIndex]
                accumulatedInterval += TimeInterval(intervalMinutes * 60)
                
                let reminderTime = targetTime.addingTimeInterval(-accumulatedInterval)
                
                // 期限時刻を超えないようにする（この条件は通常は常にtrueだが、念のため）
                if reminderTime > targetTime {
                    break
                }
                
                // リマインド終了時刻を超えないようにする
                if let reminderEndTime = reminderEndTime, reminderTime > reminderEndTime {
                    break
                }
                
                // 現在時刻より未来の時刻のみ追加
                if reminderTime > Date() {
                    reminderTimes.append(reminderTime)
                }
            }
            
            // 計算したリマインド時刻をスケジュール（期限時刻に近い順から）
            for reminderTime in reminderTimes.reversed() {
                if !task.isCompleted {
                    try await notificationManager.scheduleReminderNotification(
                        for: task,
                        at: reminderTime
                    )
                }
            }
            
            print("リマインドスケジュール完了: \(task.title ?? "無題") - スケジュール数: \(reminderTimes.count)")
        } else {
            // 従来のロジック（開始時刻から順に間隔を加算）
            var currentTime = startTime
            var notificationCount = 0

            // 直近の通知をスケジュール
            while notificationCount < maxNotificationsPerTask {
                // タスクが完了していない場合のみ通知をスケジュール
                if !task.isCompleted {
                    let interval = intervals[notificationCount % intervals.count]
                    currentTime = currentTime.addingTimeInterval(TimeInterval(interval * 60))

                    // 終了日時がある場合のみ、終了時刻をチェック
                    if let endTime = endTime, currentTime > endTime {
                        break
                    }
                    
                    // 現在時刻より未来の時刻のみスケジュール
                    guard currentTime > Date() else {
                        // 過去の時刻はスキップして次の間隔を試す
                        print("警告: リマインド時刻が過去のためスキップ: \(currentTime) (タスク: \(task.title ?? "無題"))")
                        notificationCount += 1
                        continue
                    }

                    try await notificationManager.scheduleReminderNotification(
                        for: task,
                        at: currentTime
                    )
                }

                notificationCount += 1
            }
            
            print("リマインドスケジュール完了: \(task.title ?? "無題") - スケジュール数: \(notificationCount)")
        }

        // 注: 終了日時がない場合、通知が配信された後、次の通知を自動的にスケジュールする
        // 実装は NotificationActionHandler と NotificationRefreshService で行う
    }
    
    // 次のリマインド通知をスケジュール（終了日時がない場合に使用）
    func scheduleNextReminder(for task: Task, from currentTime: Date) async throws {
        guard task.reminderEnabled else { return }
        guard !task.isCompleted else { return }
        
        // 終了日時がない場合のみ、次の通知をスケジュール
        let endTime = task.reminderEndTime ?? task.deadline ?? task.startDateTime
        guard endTime == nil else { return }
        
        let intervals = calculateReminderIntervals(for: task)
        let interval = intervals.first ?? 60  // デフォルトは1時間間隔
        
        let nextTime = currentTime.addingTimeInterval(TimeInterval(interval * 60))
        
        // 未来の時刻のみスケジュール
        guard nextTime > Date() else { return }
        
        try await notificationManager.scheduleReminderNotification(
            for: task,
            at: nextTime
        )
    }
    
    // リマインド間隔の計算（重要度とタスクタイプに基づく）
    private func calculateReminderIntervals(for task: Task) -> [Int] {
        guard let priorityString = task.priority,
              let priority = Priority(rawValue: priorityString),
              let taskTypeString = task.taskType,
              let taskType = TaskType(rawValue: taskTypeString) else {
            // デフォルト: タスクに設定されている間隔を使用、なければ1時間間隔
            return [Int(task.reminderInterval)]
        }
        
        switch (priority, taskType) {
        case (.low, .task), (.medium, .task), (.high, .task):
            // タスクタイプの場合、ユーザーが設定した間隔を優先的に使用
            // これにより、「デフォルト設定を使用」がOFFの場合に設定したカスタム間隔が反映される
            // 注意: 「デフォルト設定を使用」がONの場合、ReminderSettingViewでデフォルト間隔が
            // task.reminderIntervalに設定されているため、その値が使用される
            return [Int(task.reminderInterval)]
            
        case (.low, .schedule):
            // 低重要度・スケジュール: 段階的リマインド
            guard let startDateTime = task.startDateTime else {
                return [60]
            }
            return calculateStagedIntervals(
                startDateTime: startDateTime,
                stages: [
                    (days: 3.0, hours: nil, interval: 1440),    // 3日前から: 1日1回
                    (days: 1.0, hours: nil, interval: 720),      // 1日前から: 12時間間隔
                    (days: nil, hours: 6.0, interval: 360),      // 6時間前から: 6時間間隔
                    (days: nil, hours: 1.0, interval: 60),      // 1時間前から: 1時間間隔
                ] as [(days: Double?, hours: Double?, interval: Int)],
                overdueInterval: 30                 // 開始日時超過後: 30分間隔
            )
            
        case (.medium, .schedule):
            // 中重要度・スケジュール: 段階的リマインド
            guard let startDateTime = task.startDateTime else {
                return [60]
            }
            return calculateStagedIntervals(
                startDateTime: startDateTime,
                stages: [
                    (days: 7.0, hours: nil, interval: 1440),     // 1週間前から: 1日1回
                    (days: 3.0, hours: nil, interval: 720),       // 3日前から: 12時間間隔
                    (days: 1.0, hours: nil, interval: 360),       // 1日前から: 6時間間隔
                    (days: nil, hours: 6.0, interval: 180),      // 6時間前から: 3時間間隔
                    (days: nil, hours: 3.0, interval: 60),       // 3時間前から: 1時間間隔
                    (days: nil, hours: 1.0, interval: 30),       // 1時間前から: 30分間隔
                ] as [(days: Double?, hours: Double?, interval: Int)],
                overdueInterval: 15                 // 開始日時超過後: 15分間隔
            )
            
        case (.high, .schedule):
            // 高重要度・スケジュール: 段階的リマインド
            guard let startDateTime = task.startDateTime else {
                return [60]
            }
            return calculateStagedIntervals(
                startDateTime: startDateTime,
                stages: [
                    (days: 7.0, hours: nil, interval: 1440),     // 1週間前から: 1日1回
                    (days: 3.0, hours: nil, interval: 720),       // 3日前から: 12時間間隔
                    (days: 1.0, hours: nil, interval: 360),       // 1日前から: 6時間間隔
                    (days: nil, hours: 6.0, interval: 180),      // 6時間前から: 3時間間隔
                    (days: nil, hours: 3.0, interval: 60),       // 3時間前から: 1時間間隔
                    (days: nil, hours: 1.0, interval: 30),       // 1時間前から: 30分間隔
                    (days: nil, hours: 0.5, interval: 15),     // 30分前から: 15分間隔
                    (days: nil, hours: 0.25, interval: 5),    // 15分前から: 5分間隔
                    (days: nil, hours: 0.083, interval: 1),    // 5分前から: 1分間隔
                ] as [(days: Double?, hours: Double?, interval: Int)],
                overdueInterval: 1                  // 開始日時超過後: 1分間隔
            )
        }
    }
    
    // 段階的リマインド間隔の計算
    private func calculateStagedIntervals(
        startDateTime: Date,
        stages: [(days: Double?, hours: Double?, interval: Int)],
        overdueInterval: Int
    ) -> [Int] {
        let now = Date()
        let timeUntilStart = startDateTime.timeIntervalSince(now)
        
        var intervals: [Int] = []
        
        // 各ステージの間隔を計算
        for stage in stages {
            let threshold: TimeInterval
            if let days = stage.days {
                threshold = days * 86400
            } else if let hours = stage.hours {
                threshold = hours * 3600
            } else {
                continue
            }
            
            if timeUntilStart > threshold {
                intervals.append(stage.interval)
            }
        }
        
        // 開始日時を過ぎた場合の間隔
        if timeUntilStart <= 0 {
            intervals.append(overdueInterval)
        }
        
        return intervals.isEmpty ? [60] : intervals
    }
}
```

### 5.3 通知管理ロジック

#### 5.3.1 NotificationManager

```swift
class NotificationManager {
    private let center = UNUserNotificationCenter.current()
    
    // 通知権限の要求
    func requestAuthorization() async throws {
        let granted = try await center.requestAuthorization(options: [
            .alert, .sound, .badge, .provisional
        ])

        guard granted else {
            throw NotificationError.authorizationDenied
        }

        // iOS 15+ で Time Sensitive 通知を使用するための準備
        // Info.plist に NSUserNotificationsUsageDescription の追加が必要
        // 「重要なタスクの通知を時間に敏感な通知として送信します」
    }
    
    // アラーム通知のスケジュール
    func scheduleAlarm(for task: Task) async throws {
        guard let alarmDateTime = task.alarmDateTime else { return }
        guard alarmDateTime > Date() else { return }
        
        let content = UNMutableNotificationContent()
        content.title = "アラーム: \(task.title)"
        content.body = "設定時刻になりました"
        content.sound = UNNotificationSound(named: UNNotificationSoundName(task.alarmSound ?? "default"))
        content.categoryIdentifier = "ALARM"
        
        // 重要度設定
        content.interruptionLevel = mapPriorityToInterruptionLevel(task.priority)
        
        // スケジュール
        let trigger = UNCalendarNotificationTrigger(
            dateMatching: Calendar.current.dateComponents(
                [.year, .month, .day, .hour, .minute],
                from: alarmDateTime
            ),
            repeats: false
        )
        
        let request = UNNotificationRequest(
            identifier: "alarm_\(task.id.uuidString)",
            content: content,
            trigger: trigger
        )
        
        try await center.add(request)
    }
    
    // リマインド通知のスケジュール
    func scheduleReminderNotification(for task: Task, at date: Date) async throws {
        guard date > Date() else { return }
        
        let content = UNMutableNotificationContent()
        content.title = "リマインド: \(task.title)"
        if let category = task.category {
            content.body = "カテゴリ: \(category.name)"
        } else {
            content.body = "タスクを確認してください"
        }
        content.sound = .default
        content.categoryIdentifier = "REMINDER"
        
        // 重要度設定
        content.interruptionLevel = mapPriorityToInterruptionLevel(task.priority)
        
        // スケジュール
        let trigger = UNCalendarNotificationTrigger(
            dateMatching: Calendar.current.dateComponents(
                [.year, .month, .day, .hour, .minute],
                from: date
            ),
            repeats: false
        )
        
        let request = UNNotificationRequest(
            identifier: "reminder_\(task.id.uuidString)_\(date.timeIntervalSince1970)",
            content: content,
            trigger: trigger
        )
        
        try await center.add(request)
    }
    
    // 通知のキャンセル
    func cancelNotifications(for task: Task) async {
        let identifiers = [
            "alarm_\(task.id.uuidString)",
            // リマインド通知のIDは動的に生成されるため、プレフィックスで削除
        ]
        
        // リマインド通知を削除するため、全ての通知を取得してフィルタリング
        let pendingRequests = await center.pendingNotificationRequests()
        let taskNotificationIds = pendingRequests
            .filter { $0.identifier.contains(task.id.uuidString) }
            .map { $0.identifier }
        
        center.removePendingNotificationRequests(withIdentifiers: taskNotificationIds)
    }
    
    // スヌーズ処理
    func snoozeReminder(for task: Task) async throws {
        // スヌーズ回数をチェック
        // 注意: スヌーズ最大回数は通知スケジューリング時ではなく、
        // 通知が配信された後にユーザーが「スヌーズ」ボタンを押した時に適用される
        let snoozeCount = Int(task.snoozeCount)
        let snoozeMaxCount = Int(task.snoozeMaxCount)
        
        guard snoozeCount < snoozeMaxCount || task.snoozeUnlimited else {
            throw NotificationError.maxSnoozeReached
        }
        
        // 日付が変わったかチェック（リセット判定）
        let today = Calendar.current.startOfDay(for: Date())
        let lastSnoozeDate = Calendar.current.startOfDay(for: task.lastSnoozeDateTime ?? Date.distantPast)
        
        if today > lastSnoozeDate {
            // 日付が変わったのでカウンターをリセット
            task.snoozeCount = 0
        }
        
        // スヌーズ回数をインクリメント
        task.snoozeCount += 1
        task.lastSnoozeDateTime = Date()
        
        // 次のリマインド時刻を計算（現在時刻 + リマインド間隔）
        let nextReminderTime = Date().addingTimeInterval(
            TimeInterval(Int(task.reminderInterval) * 60)
        )
        
        // 次のリマインド通知をスケジュール
        try await scheduleReminderNotification(for: task, at: nextReminderTime)
    }
    
    // 通知が配信された後、次の通知をスケジュール（終了日時がない場合）
    func scheduleNextReminderAfterDelivery(for task: Task, deliveredAt: Date) async throws {
        guard task.reminderEnabled else { return }
        guard !task.isCompleted else { return }
        
        // 終了日時がない場合のみ、次の通知をスケジュール
        let endTime = task.reminderEndTime ?? task.deadline ?? task.startDateTime
        guard endTime == nil else { return }
        
        let reminderService = ReminderService(notificationManager: self)
        try await reminderService.scheduleNextReminder(for: task, from: deliveredAt)
    }
    
    // 重要度を通知重要度にマッピング（iOS 15+）
    private func mapPriorityToInterruptionLevel(_ priority: Priority) -> UNNotificationInterruptionLevel {
        switch priority {
        case .high:
            return .timeSensitive  // 時間に敏感な通知（Focus Modeでも表示される可能性が高い）
        case .medium:
            return .active  // 通常の通知
        case .low:
            return .passive  // 控えめな通知
        }
    }

    // 注: .critical は健康・安全関連アプリのみが使用可能な特別な権限であり、
    // タスク管理アプリでは使用できません。
    // .timeSensitive で十分な効果が得られます。
}
```

**スヌーズ最大回数の動作:**
- スヌーズ最大回数は、通知が配信された後にユーザーが「スヌーズ」ボタンを押した時に適用される
- 通知スケジュール時には使用されない
- 例: スヌーズ最大回数が5回の場合、通知が届いた後に最大5回までスヌーズできる
- 6回目のスヌーズを試みると、`NotificationError.maxSnoozeReached`エラーが発生する
- 日付が変わるとスヌーズカウンターは自動的にリセットされる
- スヌーズは、通知が届いた後に「スヌーズ」ボタンを押した時に動作し、現在時刻 + リマインド間隔の時刻に次の通知をスケジュールする

### 5.4 タスク細分化ロジック

#### 5.4.1 TaskSubdivisionService

```swift
class TaskSubdivisionService {
    private let apiClient: LLMAPIClient
    private let taskManager: TaskManager

    // UserDefaults キー
    private let lastSubdivisionDateKey = "lastSubdivisionDate"
    private let subdivisionCountKey = "subdivisionCount"

    // 細分化提案の生成
    func generateSubdivisionProposals(for task: Task) async throws -> [SubdivisionProposal] {
        // 1. 使用量制限をチェック
        guard canSubdivide() else {
            throw APIError.rateLimitExceeded
        }

        // 2. LLM APIを呼び出してタスクを細分化
        let response = try await apiClient.subdivideTask(task.title)

        // 3. 使用回数をインクリメント
        incrementSubdivisionCount()

        // 4. レスポンスをパース
        let proposals = response.subtasks.map { subtaskTitle in
            SubdivisionProposal(
                title: subtaskTitle,
                isSelected: true
            )
        }

        return proposals
    }

    // 提案を承認してタスクを作成
    func approveProposals(_ proposals: [SubdivisionProposal], parentTask: Task) async throws {
        for proposal in proposals where proposal.isSelected {
            let subtask = Task(
                title: proposal.title,
                category: parentTask.category,
                priority: parentTask.priority,
                taskType: parentTask.taskType,
                deadline: parentTask.deadline,
                startDateTime: parentTask.startDateTime,
                reminderEnabled: parentTask.reminderEnabled,
                reminderInterval: parentTask.reminderInterval
            )

            try await taskManager.createTask(subtask)
        }
    }

    // 細分化可能かどうかをチェック（1日5回制限）
    private func canSubdivide() -> Bool {
        let calendar = Calendar.current
        let today = calendar.startOfDay(for: Date())

        // UserDefaultsから前回の使用日と使用回数を取得
        let lastDateString = UserDefaults.standard.string(forKey: lastSubdivisionDateKey)
        let count = UserDefaults.standard.integer(forKey: subdivisionCountKey)

        // 日付が変わっているかチェック
        if let lastDateString = lastDateString,
           let lastDate = ISO8601DateFormatter().date(from: lastDateString) {
            let lastDay = calendar.startOfDay(for: lastDate)

            if today > lastDay {
                // 日付が変わったのでリセット
                resetSubdivisionCount()
                return true
            }
        } else {
            // 初回利用の場合
            resetSubdivisionCount()
            return true
        }

        // 5回未満なら利用可能
        return count < 5
    }

    // 使用回数をインクリメント
    private func incrementSubdivisionCount() {
        let calendar = Calendar.current
        let today = calendar.startOfDay(for: Date())
        let todayString = ISO8601DateFormatter().string(from: today)

        let count = UserDefaults.standard.integer(forKey: subdivisionCountKey)
        UserDefaults.standard.set(todayString, forKey: lastSubdivisionDateKey)
        UserDefaults.standard.set(count + 1, forKey: subdivisionCountKey)
    }

    // 使用回数をリセット
    private func resetSubdivisionCount() {
        let calendar = Calendar.current
        let today = calendar.startOfDay(for: Date())
        let todayString = ISO8601DateFormatter().string(from: today)

        UserDefaults.standard.set(todayString, forKey: lastSubdivisionDateKey)
        UserDefaults.standard.set(0, forKey: subdivisionCountKey)
    }

    // 残り使用可能回数を取得（UI表示用）
    func getRemainingSubdivisionCount() -> Int {
        guard canSubdivide() else { return 0 }

        let count = UserDefaults.standard.integer(forKey: subdivisionCountKey)
        return max(0, 5 - count)
    }
}
```

#### 5.4.2 LLMAPIClient

```swift
class LLMAPIClient {
    private let apiKey: String
    private let baseURL: URL
    private let session: URLSession
    
    struct SubdivisionResponse: Codable {
        let subtasks: [String]
    }
    
    // タスク細分化API呼び出し
    func subdivideTask(_ taskTitle: String) async throws -> SubdivisionResponse {
        // APIエンドポイント（例: OpenAI API）
        let url = baseURL.appendingPathComponent("/v1/chat/completions")
        
        var request = URLRequest(url: url)
        request.httpMethod = "POST"
        request.setValue("Bearer \(apiKey)", forHTTPHeaderField: "Authorization")
        request.setValue("application/json", forHTTPHeaderField: "Content-Type")
        
        // プロンプト構築
        let prompt = """
        以下のタスクを3-5個の小さなタスクに細分化してください。
        各タスクは具体的で実行可能なものにしてください。
        
        タスク: \(taskTitle)
        
        出力形式: JSON配列（各要素はタスク名の文字列）
        """
        
        let body: [String: Any] = [
            "model": "gpt-3.5-turbo",
            "messages": [
                ["role": "user", "content": prompt]
            ],
            "temperature": 0.7,
            "max_tokens": 500
        ]
        
        request.httpBody = try JSONSerialization.data(withJSONObject: body)
        
        // リクエスト実行（タイムアウト10秒）
        let (data, response) = try await session.data(for: request)
        
        guard let httpResponse = response as? HTTPURLResponse,
              httpResponse.statusCode == 200 else {
            throw APIError.invalidResponse
        }
        
        // レスポンスをパース
        let decoder = JSONDecoder()
        let apiResponse = try decoder.decode(OpenAIResponse.self, from: data)
        
        // レスポンスからタスクリストを抽出
        guard let content = apiResponse.choices.first?.message.content else {
            throw APIError.invalidResponse
        }
        
        // JSON配列をパース
        let subtasks = try parseSubtaskList(from: content)
        
        return SubdivisionResponse(subtasks: subtasks)
    }
    
    private func parseSubtaskList(from content: String) throws -> [String] {
        // JSON配列をパースする実装
        // 例: ["タスク1", "タスク2", "タスク3"]
        let jsonData = content.data(using: .utf8)!
        return try JSONDecoder().decode([String].self, from: jsonData)
    }
}
```

### 5.5 自然言語解析ロジック

#### 5.5.1 NaturalLanguageParser

```swift
class NaturalLanguageParser {
    private let detector: NSDataDetector
    
    init() {
        detector = try! NSDataDetector(types: NSTextCheckingResult.CheckingType.date.rawValue)
    }
    
    struct ExtractionResult {
        let dateTime: Date?
        let taskType: TaskType?
    }
    
    // 自然言語から日時を抽出
    func extractDateTime(from text: String) -> ExtractionResult {
        let range = NSRange(text.startIndex..., in: text)
        let matches = detector.matches(in: text, options: [], range: range)
        
        guard let match = matches.first,
              let date = match.date else {
            return ExtractionResult(dateTime: nil, taskType: nil)
        }
        
        // タスクタイプの判定
        let taskType: TaskType
        if text.contains("まで") || text.contains("期限") || text.contains("締切") {
            taskType = .task
        } else if text.contains("に") || text.contains("時") || text.contains("開始") {
            taskType = .schedule
        } else {
            // デフォルトはタスク
            taskType = .task
        }
        
        return ExtractionResult(dateTime: date, taskType: taskType)
    }
}
```

### 5.6 カテゴリ管理ロジック

#### 5.6.1 CategoryManager

```swift
class CategoryManager {
    private let viewContext: NSManagedObjectContext
    
    init(viewContext: NSManagedObjectContext) {
        self.viewContext = viewContext
    }
    
    // カテゴリを名前で検索または作成
    func findOrCreateCategory(name: String, color: String = "#007AFF") -> Category {
        // 既存のカテゴリを検索
        let request: NSFetchRequest<Category> = Category.fetchRequest()
        request.predicate = NSPredicate(format: "name == %@", name)
        request.fetchLimit = 1
        
        if let existingCategory = try? viewContext.fetch(request).first {
            return existingCategory
        }
        
        // 新規作成
        let category = Category(context: viewContext)
        category.id = UUID()
        category.name = name
        category.color = color
        category.createdAt = Date()
        category.usageCount = 0
        
        return category
    }
    
    // カテゴリ一覧を取得（使用回数順）
    func fetchCategories() -> [Category] {
        let request: NSFetchRequest<Category> = Category.fetchRequest()
        request.sortDescriptors = [
            NSSortDescriptor(key: "usageCount", ascending: false),
            NSSortDescriptor(key: "createdAt", ascending: false)
        ]
        
        do {
            return try viewContext.fetch(request)
        } catch {
            print("カテゴリ取得エラー: \(error)")
            return []
        }
    }
    
    // カテゴリ名で検索（オートコンプリート用）
    func searchCategories(query: String) -> [Category] {
        let request: NSFetchRequest<Category> = Category.fetchRequest()
        request.predicate = NSPredicate(format: "name CONTAINS[cd] %@", query)
        request.sortDescriptors = [
            NSSortDescriptor(key: "usageCount", ascending: false),
            NSSortDescriptor(key: "createdAt", ascending: false)
        ]
        
        do {
            return try viewContext.fetch(request)
        } catch {
            print("カテゴリ検索エラー: \(error)")
            return []
        }
    }
    
    // カテゴリを削除
    func deleteCategory(_ category: Category) throws {
        viewContext.delete(category)
        try viewContext.save()
    }
    
    // カテゴリの色を更新
    func updateCategoryColor(_ category: Category, color: String) throws {
        category.color = color
        try viewContext.save()
    }
    
    // カテゴリ名を更新
    func updateCategoryName(_ category: Category, name: String) throws {
        category.name = name
        try viewContext.save()
    }
}
```

**機能詳細**
- **カテゴリの検索・作成**: 名前で既存カテゴリを検索し、存在しない場合は新規作成
- **カテゴリ一覧取得**: 使用回数順にソートして取得（よく使うカテゴリを上位に表示）
- **カテゴリ検索**: オートコンプリート用の部分一致検索
- **カテゴリ削除**: Core Dataからカテゴリを削除（関連タスクとのリレーションはCore Dataの削除ルールで処理）
- **カテゴリ更新**: 名前と色の更新をサポート

**カテゴリの色管理**
- デフォルト色のプリセットを提供（8色）
- HEXコード形式で保存
- SwiftUIのColor型への変換機能を提供

### 5.7 繰り返しタスク生成ロジック

#### 5.7.1 RepeatingTaskGenerator

**設計方針: ローリング生成方式**
- タスク登録時に全てのインスタンスを生成するのではなく、常に「次の2-3回分」のインスタンスを維持
- 1つ完了したら、次のインスタンスを生成
- iOS通知64個制限にも対応

```swift
class RepeatingTaskGenerator {
    private let taskManager: TaskManager
    private let notificationManager: NotificationManager

    // 繰り返しタスクの初期化（最初の2-3回分を生成）
    func initializeRepeatingTask(for parentTask: Task) async throws {
        guard parentTask.isRepeating,
              let pattern = parentTask.repeatPattern else { return }

        // 最初の3回分のインスタンスを生成
        let maxInstances = 3
        var generatedCount = 0
        var currentDate = parentTask.deadline ?? parentTask.startDateTime ?? Date()

        while generatedCount < maxInstances {
            let nextDate = calculateNextDate(from: currentDate, pattern: pattern)

            // 繰り返し終了日時のチェック
            if let endDate = parentTask.repeatEndDate, nextDate > endDate {
                break
            }

            // 新しいインスタンスを作成
            let nextTask = createTaskInstance(
                from: parentTask,
                nextDate: nextDate,
                pattern: pattern
            )

            try await taskManager.createTask(nextTask)

            currentDate = nextDate
            generatedCount += 1
        }
    }

    // タスク完了時に次のインスタンスを生成（ローリング方式）
    func onTaskCompleted(for task: Task) async throws {
        guard task.isRepeating,
              let pattern = task.repeatPattern else { return }

        // 現在の未完了インスタンス数を確認
        let parentId = task.parentTaskId ?? task.id
        let pendingInstances = try await taskManager.fetchPendingRepeatingInstances(
            parentTaskId: parentId
        )

        // 未完了が2個未満なら、次のインスタンスを生成
        if pendingInstances.count < 2 {
            // 最後のインスタンスの日時から次の日時を計算
            let lastDate = pendingInstances.last?.deadline
                ?? pendingInstances.last?.startDateTime
                ?? task.deadline
                ?? task.startDateTime
                ?? Date()

            let nextDate = calculateNextDate(from: lastDate, pattern: pattern)

            // 繰り返し終了日時のチェック
            if let endDate = task.repeatEndDate, nextDate > endDate {
                return // 繰り返し終了
            }

            // 新しいインスタンスを作成
            let nextTask = createTaskInstance(
                from: task,
                nextDate: nextDate,
                pattern: pattern
            )

            try await taskManager.createTask(nextTask)
        }
    }

    // タスクインスタンスの作成
    private func createTaskInstance(
        from parentTask: Task,
        nextDate: Date,
        pattern: RepeatPattern
    ) -> Task {
        return Task(
            title: parentTask.title,
            category: parentTask.category,
            priority: parentTask.priority,
            taskType: parentTask.taskType,
            deadline: parentTask.taskType == .task ? nextDate : nil,
            startDateTime: parentTask.taskType == .schedule ? nextDate : nil,
            alarmEnabled: parentTask.alarmEnabled,
            alarmDateTime: parentTask.alarmDateTime.map {
                addIntervalToAlarm($0, nextDate: nextDate, originalDate: parentTask.deadline ?? parentTask.startDateTime!)
            },
            reminderEnabled: parentTask.reminderEnabled,
            reminderInterval: parentTask.reminderInterval,
            isRepeating: true,
            repeatPattern: pattern,
            repeatEndDate: parentTask.repeatEndDate,
            parentTaskId: parentTask.parentTaskId ?? parentTask.id
        )
    }

    // アラーム時刻の調整
    private func addIntervalToAlarm(_ originalAlarm: Date, nextDate: Date, originalDate: Date) -> Date {
        let interval = originalAlarm.timeIntervalSince(originalDate)
        return nextDate.addingTimeInterval(interval)
    }

    // 次回日時の計算
    private func calculateNextDate(from date: Date, pattern: RepeatPattern) -> Date {
        let calendar = Calendar.current

        switch pattern.type {
        case .daily:
            return calendar.date(byAdding: .day, value: 1, to: date)!

        case .weekly:
            return calendar.date(byAdding: .weekOfYear, value: 1, to: date)!

        case .monthly:
            return calendar.date(byAdding: .month, value: 1, to: date)!

        case .yearly:
            return calendar.date(byAdding: .year, value: 1, to: date)!

        case .everyNDays:
            guard let interval = pattern.interval else { return date }
            return calendar.date(byAdding: .day, value: interval, to: date)!

        case .nthWeekdayOfMonth:
            guard let weekday = pattern.weekday, let week = pattern.week else { return date }
            return calculateNthWeekday(weekday: weekday, week: week, from: date)

        case .custom:
            // カスタムパターンの処理
            return date
        }
    }

    // 毎月第N曜日の計算
    private func calculateNthWeekday(weekday: Int, week: Int, from date: Date) -> Date {
        let calendar = Calendar.current
        var components = calendar.dateComponents([.year, .month], from: date)

        // 次の月に進む
        components.month! += 1

        let firstDayOfMonth = calendar.date(from: components)!
        let firstWeekday = calendar.component(.weekday, from: firstDayOfMonth)

        // 目的の曜日までの日数を計算
        var daysToAdd = (weekday - firstWeekday + 7) % 7
        daysToAdd += (week - 1) * 7

        return calendar.date(byAdding: .day, value: daysToAdd, to: firstDayOfMonth)!
    }
}
```

**ポイント:**
1. 常に未完了の繰り返しインスタンスが2-3個ある状態を維持
2. タスク完了時に次のインスタンスを動的に生成
3. 通知数も制限内に収まりやすい
4. ユーザーからは「将来のタスクが自動で現れる」ように見える

**エッジケースの処理:**

1. **親タスク削除時の処理:**
   - 親タスクが削除された場合、関連する全ての子タスク（繰り返しインスタンス）も一緒に削除（カスケード削除）
   - Core Dataのdelete ruleを`Cascade`に設定することで自動的に削除される
   - TaskManager.deleteTask()の実装:
     ```swift
     func deleteTask(_ task: Task) async throws {
         // 1. 通知をキャンセル
         try await notificationManager.cancelNotifications(for: task)

         // 2. 繰り返しタスクの場合、全ての子インスタンスを取得して削除
         if task.isRepeating {
             let parentId = task.parentTaskId ?? task.id
             let childTasks = try await repository.fetchRepeatingInstances(parentTaskId: parentId)
             for childTask in childTasks {
                 try await notificationManager.cancelNotifications(for: childTask)
                 try await repository.delete(childTask)
             }
         }

         // 3. タスク本体を削除
         try await repository.delete(task)
     }
     ```

2. **期限超過時の処理:**
   - タスクが完了されずに期限を過ぎた場合でも、次のインスタンスは自動生成される
   - 未完了の期限超過タスクは残り続け、ユーザーが手動で完了または削除するまで存在する
   - 次のインスタンスは期限超過の有無に関わらず生成される
   - これにより、ユーザーが古いタスクを忘れても新しいインスタンスは表示され続ける

3. **parentTaskIdがnilの場合の処理:**
   - `onTaskCompleted()`メソッド内で既にフォールバック処理を実装済み:
     ```swift
     let parentId = task.parentTaskId ?? task.id
     ```
   - parentTaskIdがnilの場合、タスク自身のIDを使用
   - これにより、最初の繰り返しタスクインスタンスでも正常に動作する

---

## 6. API設計

### 6.1 LLM API仕様

#### 6.1.1 エンドポイント
- **URL**: `https://api.openai.com/v1/chat/completions`（例）
- **メソッド**: POST
- **認証**: Bearer Token

#### 6.1.2 リクエスト
```json
{
  "model": "gpt-3.5-turbo",
  "messages": [
    {
      "role": "user",
      "content": "以下のタスクを3-5個の小さなタスクに細分化してください。\n各タスクは具体的で実行可能なものにしてください。\n\nタスク: 部屋の掃除\n\n出力形式: JSON配列（各要素はタスク名の文字列）"
    }
  ],
  "temperature": 0.7,
  "max_tokens": 500
}
```

#### 6.1.3 レスポンス
```json
{
  "id": "chatcmpl-xxx",
  "object": "chat.completion",
  "created": 1234567890,
  "model": "gpt-3.5-turbo",
  "choices": [
    {
      "index": 0,
      "message": {
        "role": "assistant",
        "content": "[\"床を掃除機でかける\", \"窓を拭く\", \"ゴミを捨てる\", \"机を整理する\"]"
      },
      "finish_reason": "stop"
    }
  ]
}
```

#### 6.1.4 エラーハンドリング
- **ネットワークエラー**: タイムアウト10秒、リトライ3回まで
- **APIエラー**: エラーメッセージをユーザーに表示
- **レスポンスパースエラー**: デフォルトの細分化案を提示

---

## 7. 通知システム設計

### 7.1 通知カテゴリ定義

```swift
extension UNNotificationCategory {
    static let alarm = UNNotificationCategory(
        identifier: "ALARM",
        actions: [
            UNNotificationAction(
                identifier: "SNOOZE",
                title: "スヌーズ",
                options: []
            ),
            UNNotificationAction(
                identifier: "COMPLETE",
                title: "完了",
                options: [.foreground]
            ),
            UNNotificationAction(
                identifier: "OPEN",
                title: "アプリを開く",
                options: [.foreground]
            )
        ],
        intentIdentifiers: [],
        options: []
    )
    
    static let reminder = UNNotificationCategory(
        identifier: "REMINDER",
        actions: [
            UNNotificationAction(
                identifier: "SNOOZE",
                title: "スヌーズ",
                options: []
            ),
            UNNotificationAction(
                identifier: "COMPLETE",
                title: "完了",
                options: [.foreground]
            ),
            UNNotificationAction(
                identifier: "STOP",
                title: "停止",
                options: []
            ),
            UNNotificationAction(
                identifier: "OPEN",
                title: "アプリを開く",
                options: [.foreground]
            )
        ],
        intentIdentifiers: [],
        options: []
    )
}
```

### 7.2 通知アクション処理

```swift
class NotificationActionHandler: NSObject, UNUserNotificationCenterDelegate {
    private let taskManager: TaskManager
    private let notificationManager: NotificationManager
    
    // 通知が配信された時（フォアグラウンドでも呼ばれる）
    func userNotificationCenter(
        _ center: UNUserNotificationCenter,
        willPresent notification: UNNotification,
        withCompletionHandler completionHandler: @escaping (UNNotificationPresentationOptions) -> Void
    ) {
        // フォアグラウンドでも通知を表示
        completionHandler([.banner, .sound, .badge])
        
        // 通知が配信された後、次の通知をスケジュール（終了日時がない場合）
        let taskId = extractTaskId(from: notification)
        if let taskId = taskId,
           let task = taskManager.fetchTask(id: taskId) {
            Task {
                try? await notificationManager.scheduleNextReminderAfterDelivery(
                    for: task,
                    deliveredAt: Date()
                )
            }
        }
    }
    
    func userNotificationCenter(
        _ center: UNUserNotificationCenter,
        didReceive response: UNNotificationResponse,
        withCompletionHandler completionHandler: @escaping () -> Void
    ) {
        let taskId = extractTaskId(from: response.notification)
        guard let task = taskManager.fetchTask(id: taskId) else {
            completionHandler()
            return
        }
        
        // 通知が配信された後、次の通知をスケジュール（終了日時がない場合）
        // スヌーズや完了以外のアクションの場合も、次の通知をスケジュール
        if response.actionIdentifier != "COMPLETE" && response.actionIdentifier != "STOP" {
            Task {
                try? await notificationManager.scheduleNextReminderAfterDelivery(
                    for: task,
                    deliveredAt: Date()
                )
            }
        }
        
        switch response.actionIdentifier {
        case "SNOOZE":
            Task {
                try? await notificationManager.snoozeReminder(for: task)
            }
            
        case "COMPLETE":
            // 確認ダイアログを表示（UI層で処理）
            showCompletionConfirmation(for: task)
            
        case "STOP":
            // 確認ダイアログを表示（UI層で処理）
            showStopConfirmation(for: task)
            
        case "OPEN":
            // アプリを開いてタスク詳細画面を表示
            openTaskDetail(task)
            
        default:
            // 通知をタップした場合も、次の通知をスケジュール
            Task {
                try? await notificationManager.scheduleNextReminderAfterDelivery(
                    for: task,
                    deliveredAt: Date()
                )
            }
            break
        }
        
        completionHandler()
    }
}
```

**終了日時がない場合の動作:**
- リマインド設定をONにして、終了日時（期限/開始日時/リマインド終了時刻）を設定していない場合、指定された間隔で無限に通知が届く
- 通知が配信された後、自動的に次の通知がスケジュールされる
- アプリ起動時やフォアグラウンド復帰時にも、次の通知を追加でスケジュールする
- iOSの64個制限に対応するため、直近の通知のみをスケジュールし、通知が配信された後に次の通知を追加する仕組みを実装

---

## 8. エラーハンドリング

### 8.1 エラー定義

```swift
enum TaskError: LocalizedError {
    case invalidTitle
    case invalidDateTime
    case conflictingDates                // 開始日時 > 期限の場合
    case pastDate                        // 過去の日時を設定
    case taskNotFound
    case saveFailed
    case invalidRepeatPattern            // 繰り返し設定が無効

    var errorDescription: String? {
        switch self {
        case .invalidTitle:
            return "タスク名を入力してください"
        case .invalidDateTime:
            return "日時設定が無効です"
        case .conflictingDates:
            return "開始日時は期限より前に設定してください"
        case .pastDate:
            return "過去の日時は設定できません"
        case .taskNotFound:
            return "タスクが見つかりません"
        case .saveFailed:
            return "タスクの保存に失敗しました"
        case .invalidRepeatPattern:
            return "繰り返し設定が無効です。パラメータを確認してください"
        }
    }
}

enum NotificationError: LocalizedError {
    case authorizationDenied
    case schedulingFailed
    case maxSnoozeReached
    case notificationLimitReached        // iOS 64個制限
    case invalidScheduleTime             // 過去の時刻
    case duplicateNotification           // 重複する通知

    var errorDescription: String? {
        switch self {
        case .authorizationDenied:
            return "通知の許可が必要です。設定アプリから通知を許可してください"
        case .schedulingFailed:
            return "通知のスケジュールに失敗しました"
        case .maxSnoozeReached:
            return "スヌーズの最大回数に達しました"
        case .notificationLimitReached:
            return "通知数の上限に達しました。古いタスクを完了してください"
        case .invalidScheduleTime:
            return "過去の時刻には通知を設定できません"
        case .duplicateNotification:
            return "既に同じ通知がスケジュールされています"
        }
    }
}

enum APIError: LocalizedError {
    case networkError
    case invalidResponse
    case timeout
    case rateLimitExceeded               // 使用量制限超過
    case invalidAPIKey                   // APIキーが無効
    case quotaExceeded                   // APIクォータ超過

    var errorDescription: String? {
        switch self {
        case .networkError:
            return "ネットワークエラーが発生しました。接続を確認してください"
        case .invalidResponse:
            return "サーバーからの応答が無効です"
        case .timeout:
            return "タイムアウトが発生しました。もう一度お試しください"
        case .rateLimitExceeded:
            return "本日の細分化回数上限に達しました。明日再度お試しください"
        case .invalidAPIKey:
            return "APIキーが無効です。開発者に連絡してください"
        case .quotaExceeded:
            return "APIの使用量上限に達しました"
        }
    }
}

enum DataError: LocalizedError {
    case corruptedData                   // データ破損
    case migrationFailed                 // マイグレーション失敗
    case storageFullError                // ストレージ容量不足

    var errorDescription: String? {
        switch self {
        case .corruptedData:
            return "データが破損しています。復元を試みてください"
        case .migrationFailed:
            return "データの移行に失敗しました"
        case .storageFullError:
            return "ストレージ容量が不足しています"
        }
    }
}
```

### 8.2 エラーハンドリング戦略

1. **ユーザーに分かりやすいエラーメッセージを表示**
2. **リトライ可能な操作は自動リトライ**
3. **致命的なエラーはログに記録**
4. **ネットワークエラーはオフライン動作を可能にする**

### 8.3 多言語対応

**エラーメッセージの多言語化:**
- 現在のエラーメッセージは日本語でハードコーディングされているが、将来的な多言語対応を考慮した設計を推奨
- `Localizable.strings`を使用してエラーメッセージを外部化
- 実装例:
  ```swift
  // Localizable.strings (Japanese)
  "task_error_invalid_title" = "タスク名を入力してください";
  "notification_error_authorization_denied" = "通知の許可が必要です。設定アプリから通知を許可してください";

  // エラー定義での使用
  var errorDescription: String? {
      switch self {
      case .invalidTitle:
          return NSLocalizedString("task_error_invalid_title", comment: "")
      case .authorizationDenied:
          return NSLocalizedString("notification_error_authorization_denied", comment: "")
      }
  }
  ```
- Phase 1では日本語のみ実装し、Phase 2以降で多言語対応を検討

---

## 9. テスト仕様

### 9.1 単体テスト

#### 9.1.1 TaskManagerテスト
- タスク作成の成功ケース
- タスク作成のバリデーションエラー
- タスク完了の成功ケース
- タスク削除の成功ケース

#### 9.1.2 ReminderServiceテスト
- リマインド間隔の計算（各重要度・タスクタイプ）
- 段階的リマインド間隔の計算
- スケジュールの開始日時前からのリマインド

#### 9.1.3 NaturalLanguageParserテスト
- 日時抽出の成功ケース
- タスクタイプの判定
- 日時抽出の失敗ケース

### 9.2 統合テスト

#### 9.2.1 タスク登録フロー
1. タスク登録画面でタスクを入力
2. 保存ボタンをタップ
3. タスク一覧に表示されることを確認
4. 通知がスケジュールされることを確認

#### 9.2.2 リマインド通知フロー
1. リマインド設定済みタスクを作成
2. 通知が送信されることを確認
3. スヌーズアクションを実行
4. 次のリマインドがスケジュールされることを確認

### 9.3 UIテスト

#### 9.3.1 タスク一覧画面
- タスクの表示
- フィルタ機能
- ソート機能
- スワイプアクション

#### 9.3.2 タスク登録画面
- 入力フィールドの動作
- 自然言語解析の動作
- プリセット時間の選択
- 保存処理

### 9.4 テストデータの準備

**デバッグビルド専用のテストデータ生成機能:**

1. **実装方針:**
   - デバッグビルドでのみ有効なテストデータ生成機能を追加
   - コンパイラフラグ（`#if DEBUG`）で本番ビルドから除外
   - 開発・テスト時の効率化を目的

2. **実装例:**
   ```swift
   #if DEBUG
   class TestDataGenerator {
       static func generateSampleTasks() {
           let sampleTasks = [
               Task(title: "買い物に行く", priority: .high, deadline: Date().addingTimeInterval(3600)),
               Task(title: "レポートを書く", priority: .medium, deadline: Date().addingTimeInterval(86400)),
               Task(title: "掃除をする", priority: .low, deadline: Date().addingTimeInterval(172800))
           ]

           for task in sampleTasks {
               try? TaskManager.shared.createTask(task)
           }
       }

       static func clearAllData() {
           // 全てのタスクを削除
           try? TaskManager.shared.deleteAllTasks()
       }
   }
   #endif
   ```

3. **使用方法:**
   - 設定画面にデバッグメニューを追加（デバッグビルドのみ表示）
   - 「サンプルデータを生成」ボタン
   - 「全データを削除」ボタン
   - テスト実行前の初期状態作成に活用

4. **注意事項:**
   - 本番ビルドには含まれない（`#if DEBUG`で除外）
   - TestFlightビルドでも無効化
   - プライバシー情報は含まない

---

## 10. パフォーマンス最適化

### 10.1 データベース最適化
- インデックスの設定（id, createdAt, deadline, startDateTime）
- バッチ処理の実装
- ページネーションの実装（大量データ対応）

### 10.2 通知最適化

#### 10.2.1 iOS通知64個制限への対応
**制約:**
- iOS では1アプリあたり最大64個のローカル通知しかスケジュールできない
- この制限を超えると、新しい通知がスケジュールされず、古い通知が削除される可能性がある

**対応策: 動的スケジューリング方式**

```swift
class NotificationRefreshService {
    private let taskManager: TaskManager
    private let reminderService: ReminderService

    // アプリ起動時に呼び出す
    func refreshNotifications() async throws {
        // 1. 現在スケジュール済みの通知数を確認
        let pendingRequests = await UNUserNotificationCenter.current()
            .pendingNotificationRequests()
        let currentCount = pendingRequests.count

        print("現在の通知数: \(currentCount)/64")

        // 2. 64個に近い場合、古い通知を削除
        if currentCount > 50 {
            // 過去の通知を削除
            let outdatedNotifications = pendingRequests
                .filter { request in
                    guard let trigger = request.trigger as? UNCalendarNotificationTrigger,
                          let nextTriggerDate = trigger.nextTriggerDate() else {
                        return false
                    }
                    return nextTriggerDate < Date()
                }

            let outdatedIds = outdatedNotifications.map { $0.identifier }
            UNUserNotificationCenter.current()
                .removePendingNotificationRequests(withIdentifiers: outdatedIds)
        }

        // 3. アクティブなタスクを重要度順に取得
        let activeTasks = taskManager.fetchActiveTasks()
        let sortedTasks = activeTasks.sorted { task1, task2 in
            // 重要度順: 高 > 中 > 低
            let priority1 = priorityValue(task1.priority)
            let priority2 = priorityValue(task2.priority)
            return priority1 > priority2
        }

        // 4. 優先度ベースで通知を割り当て
        var remainingSlots = 64 - currentCount

        for task in sortedTasks {
            guard remainingSlots > 0 else { break }

            // 既存の通知数を確認
            let taskNotifications = pendingRequests.filter {
                $0.identifier.contains(task.id.uuidString)
            }

            // 重要度に応じた最大通知数を取得
            let maxNotifications: Int
            switch task.priority {
            case .high:
                maxNotifications = 15
            case .medium:
                maxNotifications = 10
            case .low:
                maxNotifications = 5
            }

            // 不足している通知数を計算
            let currentTaskCount = taskNotifications.count
            let needed = max(0, maxNotifications - currentTaskCount)

            // 残り枠の範囲内で通知を追加
            if needed > 0 && currentTaskCount < maxNotifications {
                try await reminderService.scheduleReminder(for: task)
                // 実際に追加された通知数を減算（最大maxNotifications個）
                remainingSlots -= min(needed, remainingSlots)
            }
        }

        print("通知リフレッシュ完了: 残り枠 \(remainingSlots)")
    }

    // 重要度を数値に変換（高>中>低）
    private func priorityValue(_ priority: Priority) -> Int {
        switch priority {
        case .high: return 3
        case .medium: return 2
        case .low: return 1
        }
    }
}
```

**実装のタイミング（段階的アプローチ）:**

**Phase 1（初期実装）:**
1. **アプリ起動時**: `application(_:didFinishLaunchingWithOptions:)` で呼び出し
2. **フォアグラウンド復帰時**: `applicationWillEnterForeground(_:)` で呼び出し
3. **タスク完了時**: 不要になった通知を削除し、他のタスクの通知を追加

**Phase 2以降（将来の拡張）:**
- **バックグラウンド更新**: `BGTaskScheduler` で定期的に実行（12時間ごとなど）
  - Info.plistに`BGTaskSchedulerPermittedIdentifiers`を追加
  - バックグラウンドタスクの登録と実行処理を実装
  - 注意: BGTaskSchedulerは確実に実行されるとは限らないため、Phase 1の実装で基本機能を確保

**Phase 1を推奨する理由:**
- BGTaskSchedulerの実装が複雑（Info.plist設定、登録処理など）
- バックグラウンド更新は確実性が低い（システム判断で実行されない可能性）
- アプリ起動時＋フォアグラウンド復帰時の更新で、ほとんどのユースケースに対応可能
- 開発初期は実装をシンプルに保つことでデバッグが容易

**優先度の管理:**
- 重要度「高」のタスク: より多くの通知を割り当て（最大15個）
- 重要度「中」のタスク: 標準（最大10個）
- 重要度「低」のタスク: 最小限（最大5個）

**全体制限（64個）への対応:**
- 全タスクの通知数の合計が64個に近づいた場合、優先度に基づいて動的に調整
- 調整の優先順位:
  1. 重要度「高」のタスク: 最優先で通知を確保（最大15個まで）
  2. 重要度「中」のタスク: 次に優先（残り枠の範囲で最大10個まで）
  3. 重要度「低」のタスク: 最後に割り当て（残り枠の範囲で最大5個まで）
- 64個制限に達した場合、重要度「低」のタスクの通知数を優先的に削減

#### 10.2.2 通知の一括スケジュール
- 複数のタスクの通知をまとめて処理
- トランザクション的な処理で一貫性を保つ

#### 10.2.3 不要な通知の削除
- タスク完了時に即座に削除
- 定期的なクリーンアップ処理

#### 10.2.4 通知IDの効率的な管理
- **アラーム通知の命名規則**: `alarm_{taskId}`
  - 例: `alarm_123e4567-e89b-12d3-a456-426614174000`
  - アラームは1つのタスクにつき1個のみなのでタイムスタンプ不要
- **リマインド通知の命名規則**: `reminder_{taskId}_{timestamp}`
  - 例: `reminder_123e4567-e89b-12d3-a456-426614174000_1704067200`
  - リマインドは複数スケジュールされるためタイムスタンプで識別
- プレフィックス（`alarm_`または`reminder_`）とタスクIDでフィルタリング可能

### 10.3 UI最適化
- リストの遅延読み込み
- 画像・アイコンのキャッシュ
- アニメーションの最適化

---

## 11. セキュリティ・プライバシー

### 11.1 データ保護
- ローカルデータの暗号化（Core Data/SwiftData）
- APIキーの安全な保存（Keychain）
- ユーザーデータの端末内保存

### 11.2 プライバシー
- LLM API呼び出し時の明示的な通知
- プライバシーポリシーの提供
- データ送信の最小化（タスク名のみ）

---

## 12. データマイグレーション戦略

### 12.1 Core Data マイグレーション

**初期バージョン（v1.0）:**
- データモデルのバージョン: 1.0
- マイグレーション不要（初版）

**将来のバージョンアップ時:**

1. **軽量マイグレーション（Lightweight Migration）を推奨:**
   - 対応可能な変更:
     - 新しい属性の追加（デフォルト値を設定）
     - 属性の削除
     - 属性名の変更（リネーミングIDを使用）
     - エンティティ名の変更（リネーミングIDを使用）
     - 新しいエンティティの追加
   - 実装:
     ```swift
     let container = NSPersistentContainer(name: "Moriminder")
     let description = container.persistentStoreDescriptions.first
     description?.shouldMigrateStoreAutomatically = true
     description?.shouldInferMappingModelAutomatically = true
     ```

2. **カスタムマイグレーションが必要なケース:**
   - 属性の型変更（例: String → Date）
   - リレーションシップの変更
   - 複雑なデータ変換が必要な場合
   - この場合は`NSMigrationManager`と`NSMappingModel`を使用

3. **マイグレーション戦略:**
   - Phase 1: 軽量マイグレーションのみ実装
   - Phase 2以降: 必要に応じてカスタムマイグレーションを実装
   - データモデルのバージョン履歴を明確に管理
   - 各バージョンでのスキーマ変更をドキュメント化

4. **バックアップとロールバック:**
   - マイグレーション前に自動バックアップを作成
   - マイグレーション失敗時のエラーハンドリング
   - ユーザーに分かりやすいエラーメッセージを表示

### 12.2 データ互換性

- バージョン間のデータ互換性を維持
- 古いバージョンのデータも読み込み可能にする
- マイグレーションは一方向のみ（ロールバック不可）

---

## 13. 実装計画

### 13.1 Phase 1: 基本機能
1. データモデル実装
2. タスクCRUD機能
3. 通知機能（アラーム・リマインド）
4. 基本UI実装

### 13.2 Phase 2: 高度な機能
1. カテゴリ機能
2. 重要度機能
3. 繰り返しタスク機能
4. 自然言語解析機能

### 13.3 Phase 3: 拡張機能
1. タスク細分化機能
2. プリセット時間機能
3. 通知アクション機能
4. 細かいUI調整

---

## 14. 付録

### 14.1 用語集
- **MVVM**: Model-View-ViewModelアーキテクチャパターン
- **Core Data**: Appleのデータ永続化フレームワーク
- **SwiftData**: iOS 17以降の新しいデータ永続化フレームワーク
- **UserNotifications**: iOSの通知フレームワーク
- **UNNotificationInterruptionLevel**: 通知の重要度レベル

### 14.2 参考資料
- Apple Developer Documentation
- SwiftUI Documentation
- Core Data Programming Guide
- UserNotifications Framework Documentation

---

## 15. 将来タスク

### 15.1 Notion連携機能

#### 15.1.1 概要
Notionで管理されているタスクとMoriminderの連動を検討する。カテゴリごとに個別のNotionデータベース（ページ）を指定して連携できる柔軟な設計とする。

#### 15.1.2 検討事項
- **同期方向**: 双方向同期（Moriminder ↔ Notion）または一方向同期（Notion → Moriminder）
- **同期対象**: 指定したカテゴリごとに、個別のNotionデータベース（ページ）を指定可能
- **認証**: Notion APIの認証方法（OAuth、内部統合トークンなど）
- **データマッピング**: MoriminderのタスクデータとNotionのデータベース構造の対応関係
- **同期タイミング**: リアルタイム同期、手動同期、定期的な自動同期
- **競合解決**: 両方で編集された場合の処理方法
- **実装複雑度**: やや複雑になる見込みのため、Phase 2以降で検討

#### 15.1.3 具体的な使用例と想定

**基本方針:**
- カテゴリごとに、Notionのデータベース（ページ）を個別に指定できる
- 連携を設定していないカテゴリは、Notionに影響を与えずローカルのみで管理される
- 1つのカテゴリに対して1つのNotionデータベースを紐付け可能

**具体的な動作例:**

1. **カテゴリとNotionデータベースの紐付け設定**
   - 例: 「仕事」カテゴリ → Notionの「仕事タスク」データベース
   - 例: 「プライベート」カテゴリ → 連携なし（ローカルのみ）
   - 例: 「家族」カテゴリ → Notionの「家族タスク」データベース（別のデータベース）
   - 設定画面で、各カテゴリごとにNotionデータベースの選択・設定が可能

2. **Notion → Moriminder（取り込み）**
   - 設定されたNotionデータベースから、指定した条件（例: 特定のプロパティやタグ）に該当するタスクを自動的に取り込む
   - 取り込まれたタスクは、対応するカテゴリとして自動的に設定される
   - 例: 「仕事タスク」データベースから取り込まれたタスクは「仕事」カテゴリとして設定される
   - 取り込みは定期的に自動実行、または手動で実行可能

3. **Moriminder → Notion（状態反映）**
   - 連携設定があるカテゴリのタスクを「進行中」や「完了」に変更すると、紐付けられたNotionデータベースの対応するタスクも自動的に更新される
   - 例: 「仕事」カテゴリのタスクを完了にすると、Notionの「仕事タスク」データベースの該当タスクも完了状態に更新される
   - 連携設定がないカテゴリのタスクは、Notionに送信されない

4. **プライバシー保護**
   - 連携設定がないカテゴリ（例: 「プライベート」など）のタスクは、Notionに送信されない
   - プライベートなタスクは完全にローカルで管理され、Notionのデータベースに反映されることはない
   - カテゴリごとに個別のデータベースを指定できるため、仕事とプライベートのタスク管理を完全に分離できる

**想定される実装:**
- カテゴリごとのNotionデータベース設定機能
- カテゴリとNotionデータベースの1対1マッピング
- Notion APIとの双方向同期（設定されたカテゴリのみ）
- 同期設定画面で、各カテゴリごとにNotion連携の有効/無効、対象データベース、同期間隔を設定可能
- カテゴリ管理画面から、Notion連携設定へのアクセスも可能

#### 15.1.4 実装時の考慮事項
- Notion APIの制限事項（レート制限、データ構造など）
- エラーハンドリング（ネットワークエラー、APIエラーなど）
- ユーザー設定（カテゴリごとの連携有効/無効、対象データベースの選択、同期間隔など）
- プライバシー・セキュリティ（APIキーの管理、データ送信の暗号化など）
- カテゴリとNotionデータベースのマッピング管理（設定の保存・読み込み）
- Notionデータベースの選択UI（データベース一覧の取得と表示）
- カテゴリ削除時の連携設定の処理

---

## 16. 変更履歴

| 日時 | バージョン | 変更内容 | 変更者 |
|------|-----------|---------|--------|
| 2025-11-09 02:23 | 1.0 | 初版作成 | - |
| 2025-11-09 03:02 | 1.1 | レビューに基づく修正:<br>- NotificationManager の criticalAlert 要求を削除、Time Sensitive に変更<br>- iOS通知64個制限への対応を追加（動的スケジューリング方式）<br>- 繰り返しタスク生成ロジックをローリング生成方式に全面改訂<br>- RepeatPattern を enum から struct に変更<br>- エラーハンドリングを拡充（TaskError、NotificationError、APIError、DataError） | - |
| 2025-11-09 03:37 | 1.2 | レビュー指摘事項への対応:<br>- 通知数の優先度管理を修正（高=15個、中=10個、低=5個）<br>- NotificationRefreshService に優先度ベースの割り当てロジックを追加<br>- RepeatPattern の Core Data 保存方法を明記（Transformable 属性）<br>- ローリング生成方式のエッジケース処理を詳細化<br>- API 使用量制限の追跡方法を実装（UserDefaults）<br>- BGTaskScheduler の段階的アプローチを明記<br>- 通知 ID の命名規則を明確化<br>- 多言語対応、データマイグレーション戦略、テストデータ生成を追加<br>- Task エンティティに lastSnoozeDateTime 属性を追加 | - |
| 2025-11-09 15:44 | 1.3 | アラーム設定のデフォルト値の修正:<br>- アラーム設定のデフォルト値を現在時刻から期限設定の時刻に変更<br>- AlarmSettingView に defaultDateTime パラメータを追加<br>- タスクタイプに応じて deadline または startDateTime をデフォルト値として使用 | - |
| 2025-11-09 15:46 | 1.4 | 日時設定の任意項目としての実装:<br>- DateSettingView にトグルによる有効/無効の切り替え機能を追加<br>- トグルがOFFの場合、期限/開始日時は未設定（nil）のまま<br>- トグルがONの場合のみDatePickerを表示<br>- プリセット時間ボタンはトグルがOFFでも表示され、タップで自動的にONになる | - |
| 2025-11-11 | 1.5 | リマインドロジックの修正と詳細化:<br>- ReminderService のリマインド間隔計算ロジックを修正（タスクタイプの場合、task.reminderIntervalを優先的に使用）<br>- reminderStartTimeが設定されていない場合の動作を追加（期限時刻から逆算してリマインドを開始）<br>- 期限時刻から逆算する場合と開始時刻から順に加算する場合の使い分けを明確化<br>- スヌーズ最大回数の動作説明を追加（通知スケジュール時ではなく、通知配信後のスヌーズ操作時に適用されることを明記）<br>- ReminderSettingViewの動作説明を追加（デフォルト設定のON/OFFの動作、既存タスク編集時の自動判定）<br>- 段階的リマインド間隔の計算でタプルの構造を修正（days: Double?, hours: Double?, interval: Int）<br>- リマインド終了時刻のチェックロジックを追加 | - |

